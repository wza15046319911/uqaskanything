"""
server.py — 问答前端的本地 API(FastAPI)
POST /api/ask {question, generate} -> qa.run 结果(JSON)
GET  /        -> web/index.html 单页前端

用法:
    python server.py            # http://localhost:8077
"""
...
# S2 开课码:出现在 2026:2 搜索页即代表该课 S2 开(见 s2_progress.md)。文件缺则空集。
_S2_FILE = pathlib.Path(__file__).parent / "s2_course_codes.txt"
...
# ---------------- 选课模拟器(确定性,无 LLM;客户端持 state,服务端无状态重放) ----------------
...
@app.get("/api/sim/program/{program_id}")
