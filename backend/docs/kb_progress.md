...
数据:`data/kb/{urls.csv, fetched_*.jsonl, raw/}`(抓取源,入库随仓库带)。

## 重建生成物(本地可再生,不入库)

`chunks.jsonl / chunks_faq.jsonl / chunks_all.jsonl / chunk_vecs.jsonl` 是派生物
(chunk_vecs 50MB+,超 GitHub 单文件软上限),已 `.gitignore`。换机器/丢失后从
抓取源(raw HTML + `fetched_*.jsonl`)本地重建,**无需重爬**(需 Ollama 跑 bge-m3):
